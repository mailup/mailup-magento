...
class MailUp_MailUpSync_Model_Observer
{
    ...
    public function leggiUtente($observer)
    {
        ...
        try {
            ...
            $result = $soapclient->ReportByUser(
                array(
                    "accessKey" => $accessKey,
            ...
            $statoRegistrazione = (string)$xml->Canali->Email;
            if (Mage::getStoreConfig('mailup_newsletter/mailup/enable_log'))
                Mage::log("stato registrazione: ".$statoRegistrazione);

            if ($statoRegistrazione) {
                // Ensure that before_save does not fire
                $this->_authenticationCalled[$model->getEmail()] = true;
                // Set subscription based on returned $statoRegistrazione
                switch (strtolower($statoRegistrazione)) {
                    case "iscritto":
                        Mage::getModel('newsletter/subscriber')
                            ->loadByEmail($model->getEmail())
                    ...
                    case "in attesa":
                        Mage::getModel('newsletter/subscriber')
                            ->loadByEmail($model->getEmail())
                            ->setStatus(Mage_Newsletter_Model_Subscriber::STATUS_UNCONFIRMED)
                            ->save();
                        Mage::getSingleton('core/session')
                            ->addNotice(
                                Mage::helper("mailup")->__("Your subscription is waiting for confirmation")
                            );
                        break;
                    default:
                        Mage::getModel('newsletter/subscriber')
                            ->loadByEmail($model->getEmail())
                            ->setStatus(Mage_Newsletter_Model_Subscriber::STATUS_UNSUBSCRIBED)->save();
    ...
    public function sendUser($observer)
    {
        ...
        if ($model->getStatus() == Mage_Newsletter_Model_Subscriber::STATUS_UNCONFIRMED) {
            Mage::getSingleton('core/session')
                ->addNotice(Mage::helper("mailup")->__("Your subscription is waiting for confirmation"));
        }
        ...
        if (($module == "customer" && $controller == "account" && $action == "createpost") ||
        ...
        try {
            ...
            if (isset($_SERVER['HTTP_CLIENT_IP'])) {
                $clientIp = $_SERVER['HTTP_CLIENT_IP'] ? : ($_SERVER['HTTP_X_FORWARDE‌​D_FOR'] ? :
                    $_SERVER['REMOTE_ADDR']);
            ...
            try {
                if (Mage::getStoreConfig('mailup_newsletter/mailup/enable_log')) {
                    ...
                    Mage::log("MAILUP | invio utente - ip client: ".$clientIp);
                }
     ...
     * @throws Exception
     */
    protected static function setCustomerForDataSync($customerId, $storeId = null)
    {
        if (Mage::getStoreConfig('mailup_newsletter/mailup/enable_log')) {
