...
class MailUp_MailUpSync_WebhookController extends Mage_Core_Controller_Front_Action
{
    ...
    public function subscribeAction()
    {
        ...
        if (!$email) {
            ...
            foreach ($_REQUEST as $k => $v) {
                ...
                $parsedVars = array();
                parse_str($k, $parsedVars);
                if ($parsedVars and isset($parsedVars["customerEmail"])) {
                    $email = $parsedVars["customerEmail"];
                    break;
                }
            }
        }
     ...
     *
     * @param string $keyStr the key as a binary string
     * @param string $dataStr the data to decrypt/encrypt as a binary string
     *
     * @return string $dataStr result of the RC4 as a binary string
    ...
    public static function rc4($keyStr, $dataStr)
    {
        ...
        for ($counter = 0; $counter < 256; $counter++) {
            $indexTwo          = ($key[$indexOne] + $state[$counter] + $indexTwo) % 256;
            $tmp             = $state[$counter];
            $state[$counter] = $state[$indexTwo];
            $state[$indexTwo]  = $tmp;
            $indexOne          = ($indexOne++) % $len;
        }
