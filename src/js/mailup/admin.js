...
{
    $('mailup_newsletter_mailup_list').observe(
        'change', function (event) {
            var currentGroupSelected = $('mailup_newsletter_mailup_default_group').value;
            var updater = new Ajax.Updater(
                'mailup_newsletter_mailup_default_group', url, {
                method: 'get',
                onSuccess: function () {
...
{
    $('mailup_selftest_button').observe(
        'click', function (event) {
            var request = new Ajax.Request(
                url, {
                    method: 'get',
                    onFailure: function (transport) {
                        $('messages').update('<ul class="messages"><li class="error-msg"><ul><li>Error checking connection details</li></ul></li></ul>')
                    },
